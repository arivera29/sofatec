 ...
 */
@WebServlet("/XmlEjecutadas")
public class XmlEjecutadas extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public XmlEjecutadas() {
        super();
			...
			if (equipo.Find(imei)) {
				if (equipo.getActivo() == 1 && !equipo.getRecurso().equals("-1")) {
					...
					if (recurso.isSupervisor(equipo.getRecurso())) {
						String sql = "select recucodi,recunomb,count(orders.orden) cnt_ejecutadas from recurso LEFT OUTER JOIN orders ON (recurso.recucodi = orders.recurso AND orders.estado = 99 AND date(fecha_cierre)=current_date() ) where recucesu=? group by recucodi";
						...
						while (rs.next()) {
							...
							out.println("<ejecutadas>" + (rs.getString("cnt_ejecutadas")==null?"0":rs.getString("cnt_ejecutadas")) + "</ejecutadas>");
							out.println("</tecnico>");
