...
public class SrvInterfazLotes extends HttpServlet {
	...
	protected void ProcesarPeticion(HttpServletRequest request,
		...
		if (lotes.length == 0) {
			out.print("Parametro lotes vacio");
			return;
		}
			...
			if (rsCierres.next()) {
				...
				do {
					...
					while (rsPasos.next()) {
						...
						String obs = formatoString(rsPasos.getString("OBSERVACION")).toUpperCase();
					...
					if (rsMedidor.next()) {
						...
						String obs = rsCierres.getString("OBSERVACION").trim().toUpperCase();
						if (obs.length() > 250) {
							cadena += obs.substring(0, 249);
						} else {
							cadena += obs;
