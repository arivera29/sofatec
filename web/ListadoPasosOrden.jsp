...
<% if (!rsPasosIsEmpty) { %>
		<table>
			...
			<tr <%= (++contador%2==0)?"class='odd'":"" %>>
				...
				<% if (!rsPasos.getString("CO_ACCEJE").equals("")) { %>
				<%
					sql = "SELECT QO_OSACCION.DESC_COD FROM QO_OSACCION WHERE NUM_PASO =? AND TIP_OS=? AND CO_ACCEJE=?";
				...
				<td>
					<%=(String) rsPasos.getString("OBSERVACION")%>
						...
						<a href="javascript:ModificarObservacion(<%= rsPasos.getString("ID") %>,1,'<%=(String) request.getParameter("tipo")%>')">Modificar</a>
					<% } %>
				</td>
				<td>
					<% if (usuario.getResolver()== 1) { %>
					<a 	href="javascript:Accion('<%=(String) request.getParameter("orden")%>','<%=(String) rsPasos.getString("NUM_PASO")%>','<%=(String) request.getParameter("tipo")%>','<%= rsPasos.getString("CO_ACCEJE") %>');" class="boton" title="Acciones">ACC</a> 
					<% } %>
				</td>
				<td>
				<% if (usuario.getResolver()== 1) { %>	
