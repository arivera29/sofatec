...
<%
	...
	String sql = "SELECT QO_ORDENES.* "
			+ " FROM QO_ORDENES " 
			+ " WHERE QO_ORDENES.ESTADO_OPER=0 " 
...
<script type="text/javascript" language="javascript">
	...
	function SolicitarNuevoLote() {
		...
		if (confirm("Esta seguro Solicitar nuevo lote para las OS seleccionadas?")) {
			$("#info").html(
					"<img src='images/loading.gif' > Procesando Solicitud")
			$.get(url, procesar);
		}
	}
...
<body>
	...
	<div class="contencenter demo">
		<h2>
			PENDIENTES LOTE
			<%=(String) request.getParameter("num_lote")%></h2>
		...
		<form action="" name="form3">
			...
			<table>
				...
				<%
					do {
				%>
				<tr id="<%=(String) rs.getString("NUM_OS")%>"
					<%=fila % 2 == 0 ? "class='odd'" : ""%>>
					<td><input type="checkbox" name="orden"
						value="<%=rs.getString("NUM_OS")%>"></td>
