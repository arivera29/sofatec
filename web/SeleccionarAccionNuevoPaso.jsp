...
<%
	...
	String flujo = (String)request.getParameter("flujo");
	String sql = "SELECT CO_ACCEJE,DESC_COD FROM QO_OSACCIONFLUJO WHERE TIP_OS = ? AND NUM_PASO=? AND TIP_FLUJO=? ORDER BY DESC_COD";
	PreparedStatement pst = conexion.getConnection().prepareStatement(sql);
	pst.setString(1, tipo);
	pst.setString(2, paso);
	pst.setString(3, flujo);
...
<script type="text/javascript">
	...
	function Seleccion(orden,paso,tipo,parent) {
		...
		var url = "reportar_nuevo_paso.jsp";
		$.post(url,{
			orden : orden,
			paso : paso,
			accion : accion,
