...
<%
	...
	String sql = "SELECT QO_ORDENES.*, RECURSO.RECUNOMB "
			...
			+ " WHERE QO_ORDENES.ESTADO_OPER=99 " 
...
<h2>RESUELTAS LOTE <%= (String)request.getParameter("num_lote") %></h2>
	...
	<tr id="<%= (String)rs.getString("NUM_OS") %>" <%=fila % 2 == 0 ? "class='odd'" : ""%>>
		...
		<td>
			<% if (usuario.getResolver()== 1) { %>
				<a href="ReporteOrdenEfectiva.jsp?orden=<%= rs.getString("NUM_OS") %>&lote=<%= rs.getString("NUM_LOTE") %>" class="boton">Modificar</a>
			<% } %>
